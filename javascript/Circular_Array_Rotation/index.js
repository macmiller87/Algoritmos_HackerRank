...
function circularArrayRotation(a, k, queries) {
    ...
    let arrayNumbers = a;
    let quantRotation = k;
    let arrayIndices = queries;
    
    let lastNumber = 0;
    let result = [];
    
    for(let i = 0; i < quantRotation; i++) {
        lastNumber = arrayNumbers.pop();
        arrayNumbers.unshift(lastNumber);
    }
    
    for(let j in arrayIndices) {
        result.push(arrayNumbers[arrayIndices[j]]);
    }
