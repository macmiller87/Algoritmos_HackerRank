...
function serviceLane(cases, width) {
    ...
    let arrayCases = cases;
    let arrayWidth = width;
    
    let arrayFilterNumbers = [];
    let result = [];
    
    for(let i in arrayCases) {
        arrayFilterNumbers.push(arrayWidth.slice(arrayCases[i][0], arrayCases[i][1] + 1));
    }
    
    for(let i in arrayFilterNumbers) {
        result.push(Math.min(...arrayFilterNumbers[i]));
    }
